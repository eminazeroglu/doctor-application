...
class DoctorService extends BaseCrudService
{
    ...
    */
    ...
    public function getNearestAvailableSlots(Doctor $doctor, int $limit = 3): \Illuminate\Support\Collection
    {
        ...
        while ($slots->count() < $limit && $current->lte($deadline)) {
            foreach ($doctor->activeClinics as $clinic) {
                $daySlots = $this->getAvailableTimeSlots($doctor, $clinic->id, $current);
    ...
    */
    ...
    {
        // 1) Bu tarixə düşən AKTİV cədvəlləri seç
        $ymd = $date->toDateString();

        /** @var \Illuminate\Support\Collection|DoctorSchedule[] $schedules */
        $schedules = DoctorSchedule::query()
            ...
            ->get()
            ->filter(function (DoctorSchedule $sch) use ($date) {
                // Frequency uyğunluğu (daily/weekly/monthly)
                return $this->scheduleMatchesDate($sch, $date);
            })
            ->values();
        ...
        $duration = $serviceId
            ...
            : (int)($doctor->consultation_duration ?: 30);

        // 3) Günün unavailability və randevuları
            ...
            ->where(function($q) use ($dayStart, $dayEnd) {
                $q->whereBetween('start_time', [$dayStart, $dayEnd])
                    ...
                    ->orWhere(function($qq) use ($dayStart, $dayEnd) {
                        $qq->where('start_time', '<=', $dayStart)->where('end_time', '>=', $dayEnd);
                    });
            })
            ->get();
        ...
        $appointments = Appointment::query()
            ->where('doctor_id', $doctor->id)
            ->where('clinic_id', $clinicId)
            ->whereDate('start_time', $ymd)
            ->whereNotIn('appointment_status', [
                AppointmentStatusEnum::Cancelled,
                AppointmentStatusEnum::NoShow
            ])->get();

        // 4) Cədvəllərin hər biri üçün slotları topla
        $available = [];

        foreach ($schedules as $sch) {
            // from_time/to_time bu gün üçün tam datetime-a çevrilir
            [$startTime, $endTime] = $this->scheduleWindowForDate($sch, $date);

            if ($date->isToday()) {
                // bütünlükdə iş vaxtı keçibsə boşdur
                if (Carbon::now()->gte($endTime)) {
                    continue;
                }
                // indi startdan böyükdürsə indidən başla – duration-a yuvarlaqla
                if (Carbon::now()->gt($startTime)) {
                    $now = Carbon::now();
                    $rounded = (int)ceil($now->minute / $duration) * $duration;
                    $startTime = $now->copy()->minute($rounded)->second(0);
                }
            }
    ...
    */
    ...
    private function isSlotAvailable(Carbon $slotStart, Carbon $slotEnd, $unavailabilities, $appointments): bool
    {
        foreach ($unavailabilities as $u) {
            if ($this->timesOverlap($slotStart, $slotEnd, $u->start_time, $u->end_time)) {
                return false;
            }
        }

        foreach ($appointments as $a) {
            if ($this->timesOverlap($slotStart, $slotEnd, $a->start_time, $a->end_time)) {
                return false;
            }
        }
    ...
    */
    ...
    {
        $doctorClinicId = $this->getDoctorClinicId($doctor->id, $clinicId);
        if ($doctorClinicId) {
            $pivot = DoctorClinicService::query()
                ->where('doctor_clinic_id', $doctorClinicId)
                ->where('service_id', $serviceId)
                ->where('is_active', true)
                ->first(['duration']);

            if ($pivot && !empty($pivot->duration)) {
                return (int)$pivot->duration;
            }
        }
